<script>
  // Service-only: resets the Management password on Management's behalf
  // (forgotten/rotation) — no current-password proof, unlike
  // ChangePasswordCard (Service's own password). See POST
  // /api/service/password/management, gated on require_level("service").
  import { lang } from './stores.js';
  import { translate } from './translations.js';
  import { changeManagementPassword, ServiceApiError } from './serviceApi.js';
  ...
  async function submit(event) {
    ...
    saving = true;
    try {
      await changeManagementPassword(next, confirm);
      success = translate($lang, 'service_management_password_updated');
      next = '';
      confirm = '';
    } catch (err) {
...
<h2>{translate($lang, 'service_change_management_password')}</h2>
